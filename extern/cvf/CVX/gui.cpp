...
//{
//}
int CVEventHandler::onEvent(int event, int param1, int param2, void *data)
{
	return event;
}
	...
	:public CVWindow
{
	...
	public:
		...
		virtual void sendEvent(int evt, int param1, int param2, void *data)
		{
			if (!data)
				data = &name;
...
private:
	...
	void _sendEvent(int evt, int param1, int param2, void *data)
	{
...
public:
	...
	static void cvxMouseCallback(int event, int x, int y, int flags, void *data)
	{
		if (CVWindowImpl *d = (CVWindowImpl*)data)
		{
			...
			d->_sendEvent(event, x, y, (void*)(long)flags);
		}
	}
	void setCallBack()
	{
...
void CVWindow::setNavigator(int keyPrev, int keyNext, int keySave, int mouseNext, bool numKeySwitch)
{
	...
	auto *wnd = this;
	this->setEventHandler([wnd, mouseNext, keyPrev, keyNext, keySave](int evt, int param1, int param2, void *data){
		if (evt == mouseNext)
			wnd->showNext();
		...
		{
			_proCVXCommand(wnd, (char_t*)data);
		}
	});
...
CVWindow* imshowx(const std::string &wndName, ImageSetPtr is[], int count, const std::string isName[], int keyPrev, int keyNext, int keySave, bool numKeySwitch, int mouseNext)
{
	...
	int cid = 0;
	wnd->setEventHandler([wnd, vis, vlabels, count, cid, keyPrev, keyNext, keySave, numKeySwitch, mouseNext](int evt, int code, int, void*) mutable{
		int ret = evt;
